...
/*
		enum (열거형) : 서로 관련있는 상수들의 집합을 나타낸다. 클래스처럼 보이게 하는 상수이다.
					    열거형은 클래스처럼 독립된 java파일에 만들 수 있고 하나의 java파일에 클래스와 같이 만들 수 있고
					    클래스 안에 내부 클래스처럼 만들 수 있다.
					   
		열거형의 속성 및 메서드
		name() : 열거형 상수의 이름을 문자열로 반환한다.
		ordinal() : 열거형 상수가 정의된 순서값(index)을 반환한다. index는 0부터 시작한다.
		valueOf("열거형상수명") : 지정된 열거형에서 '열거형상수명'과 일치하는 열거형 상수를 반환한다.
		열거형이름.상수명 : valueOf()메서드와 같다.
		
		열거형 선언하기
		방법1) enum 열거형이름 {상수명1, 2, 3}
		방법2) enum 열거형이름 {
...
public class EnumTest {
	...
	public enum Season {	// 상수명(값들) 형식의 선언
		...
		// 생성자
		Season(String months, int num) {	// 컴파일러가 private을 붙혀준다.
			span = months;
			this.num = num;
		}
...
//		if (myCol == myCnt) {
		...
		// 열거형이름.values() : 모든 상수들을 배열형태로 가져온다.
